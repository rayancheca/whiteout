...
| T-116 | A run can stall to a dead stop instead of ending | A stall now ends the run: `RunOutcome.hasStalled` is a second disjunct of `hasSettled`, the card reads **RAN OUT OF SPEED**, and `endedInCrash` is `false`. Verified on the simulator at the original 894 m stall. See ADR-027 | — | done |
| T-119 | A stall is legible but far too common | Ending the run made the soft-lock visible, not rare: **46.2% of never-tuck runs stall** (8.7% across all input tapes), and the earliest is 17.8 m and 5.9 s from the drop-in — a card that says "17 m" is a broken game, not a short run. The two candidate causes are measured and separable: the ridge octave (amplitude 78, frequency 0.0016) locally exceeds the base −0.30 gradient, and the 8 m/s drop-in buys only 3.3 m of climb. Done when a no-tuck run on any seed and surface reaches at least a few hundred metres before it can stall, without flattening the mountain's character. **Changing terrain moves every distance in `STATE.md` and every README screenshot — budget for that** | T-116 | todo |
